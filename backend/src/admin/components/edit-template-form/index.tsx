...
// Define the update data interface (same as backend)
interface DigitalProductUpdateData {
    name?: string
    medias?: { type: MediaType; file_id: string; mime_type: string }[]
    template_data?: any
    ...
    show_in_studio?: boolean
    isTemplate?: boolean
    product?: {
        title: string
        options?: { title: string; values: string[] }[]
        variants?: { title: string; options: { [key: string]: string }; prices: any[] }[]
    }
}
...
const EditTemplateForm = ({ initialData, onSuccess }: Props) => {
    ...
    const [productTitle, setProductTitle] = useState(initialData.product?.title || "")
    const [loading, setLoading] = useState(false)
    const [templateData, setTemplateData] = useState(
        initialData.template_data
            ? typeof initialData.template_data === "object"
                ? JSON.stringify(initialData.template_data, null, 2)
                : initialData.template_data
            : ""
    )
    ...
    const [categoryTop, setCategoryTop] = useState(initialData.category_top || "")
    const [categorySub, setCategorySub] = useState(initialData.category_sub || "")
    const [tags, setTags] = useState(initialData.tags?.join(", ") || "")
    const [thumbnail, setThumbnail] = useState(initialData.thumbnail || "")
    const [thumbnailFile, setThumbnailFile] = useState<File | null>(null)
    const [thumbnailPreview, setThumbnailPreview] = useState(initialData.thumbnail || "")
    const [showInStudio, setShowInStudio] = useState(initialData.show_in_studio || false)
    const [categoryTopMode, setCategoryTopMode] = useState<"select" | "add">("add")
    const [categorySubMode, setCategorySubMode] = useState<"select" | "add">("add")
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            // Include existing medias (unchanged)
            medias.forEach((media) => {
                if (media.file_id && media.mime_type) {
                    mediaData.push({
                        type: media.type,
                        file_id: media.file_id,
                        mime_type: media.mime_type,
            ...
            // Handle template data: JSON, URL, or null
            let parsedTemplateData: any = null
            if (templateData) {
                if (isValidUrl(templateData)) {
                    // If it's a valid URL (including S3 URLs), keep it as a string
                    ...
                    try {
                        ...
                        toast.error("Invalid JSON", {
                            description: "Template data must be valid JSON or a valid URL"
                        })
                        setLoading(false)
                        return
                    }
            ...
            const updatePayload: DigitalProductUpdateData = {
                name,
                medias: mediaData,
            ...
            const response = await fetch(`/admin/digital-products/${initialData.id}`, {
                method: "PUT",
                credentials: "include",
                headers: {
            ...
            toast.error("Error", {
                description: `An error occurred while updating the template: ${(e as Error).message}`
            })
        } finally {
            setLoading(false)
        }
            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <div className="space-y-3">
                        {initialData.thumbnail && !thumbnailFile && !thumbnail && (
                            <div className="mb-2">
                                <label className="block text-sm mb-1">Current Thumbnail</label>
                                <img 
                ...
                <div>
                    <label className="block mb-1">Template Data (JSON / URL)</label>
                    <Textarea
                        name="template_data"
                        placeholder='{"key": "value"} or https://example.com'
                        value={templateData}
                ...
                <fieldset>
                    ...
                    {medias.map((media, index) => (
                        <fieldset key={index} className="my-2 p-2 border border-solid rounded">
                            ...
                            {media.file_id ? (
                                <div className="mt-2">
                                    <p>Existing: {media.file_id} ({media.mime_type})</p>
                                    <Button
                                        type="button"
                                        variant="secondary"
                                        onClick={() => changeFiles(index, { file: undefined, file_id: undefined, mime_type: undefined })}
                                        className="mt-2"
                                    >
                                        Replace File
                                    </Button>
