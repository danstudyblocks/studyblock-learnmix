...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
    try {
        ...
        const templates = digitalProducts
            .filter(product => {
                // Must be a template with template_data
                if (!product?.isTemplate || !product?.template_data) {
                    return false;
                }
                
                // Exclude blocks - blocks are identified by category_top, category_sub, or tags containing "block"
                const isBlock = 
                ...
                // Return true only if it's NOT a block
                return !isBlock;
            })
            .map(product => ({
                id: product.id,
